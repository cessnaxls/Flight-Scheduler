window.addEventListener('error', (e)=>{const b=document.getElementById('bootError');if(b){b.style.display='block';b.textContent='LineForge startup error: '+(e.message||'Unknown JavaScript error')+' at '+(e.filename||'app.js')+':'+(e.lineno||'?');}});
window.addEventListener('unhandledrejection',(e)=>{const b=document.getElementById('bootError');if(b){b.style.display='block';b.textContent='LineForge startup error: '+(e.reason?.message||String(e.reason||'Unhandled promise rejection'));}});
(() => {
'use strict';
